    ...
    return (
        <div className="contact">
            ...
            <form ref={form} onSubmit={handleSubmit(onSubmit)} className="pb-3">
                ...
                <textarea name="content" placeholder="Your Message.." cols="30" rows="5" {...register("content", { required: "No message to send" })}></textarea>
                {errors.content && <p className="text-center text-danger fs-5 my-0 ">{errors.content.message}</p>}
                <button type="submit" className="btn-mail">Send Mail</button>
            </form>
        </div>
    );
};
